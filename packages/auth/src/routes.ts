...
export function createAuthRoutes(config: AuthConfig) {
    ...
    return {
        ...
        async GET(req: VattsRequest, params: { [key: string]: string }) {
            ...
            for (const { provider, route: additionalRoute } of additionalRoutes) {
                ...
                if (additionalRoute.method === 'GET' && additionalRoute.path.includes(route)) {
                    try {
                        ...
                        return VattsResponse.json({ error: 'Provider route error' }, { status: 500 });
                    }
                }
            }
            ...
            switch (route) {
                ...
                default:
                    return VattsResponse.json({ error: 'Route not found' }, { status: 404 });
            }
        },

        async POST(req: VattsRequest, params: { [key: string]: string }) {
            const path = params["value"];
            const route = Array.isArray(path) ? path.join('/') : path || '';
...
async function handleSession(req: VattsRequest, auth: HWebAuth) {
    ...
    if (!session) {
        return VattsResponse.json({ session: null });
    }

    return VattsResponse.json({ session });
}
