...
export class VattsResponse {
     ...
     */
    status(code: number): VattsResponse {
        this._status = code;
        return this;
    }
     ...
     */
    header(name: string, value: string): VattsResponse {
     ...
     */
    headers(headers: Record<string, string>): VattsResponse {
     ...
     * Define um cookie
     */
    cookie(name: string, value: string, options?: CookieOptions): VattsResponse {
     ...
     * Remove um cookie
     */
    clearCookie(name: string, options?: CookieOptions): VattsResponse {
        this._cookies.push({
            name,
            value: '',
     ...
     */
    text(data: string): VattsResponse {
     ...
     * Envia resposta HTML
     */
    html(data: string): VattsResponse {
     ...
     */
    redirect(url: string, status: number = 302): VattsResponse {
        this._status = status;
     ...
     */
    static json(data: any, options?: { status?: number, headers?: Record<string, string> }): VattsResponse {
        const response = new VattsResponse();
     ...
     */
    static text(data: string, options?: { status?: number, headers?: Record<string, string> }): VattsResponse {
        const response = new VattsResponse();
        if (options?.status) response.status(options.status);
        if (options?.headers) response.headers(options.headers);
     ...
     */
    static html(data: string, options?: { status?: number, headers?: Record<string, string> }): VattsResponse {
        const response = new VattsResponse();
        if (options?.status) response.status(options.status);
        if (options?.headers) response.headers(options.headers);
     ...
     */
    static redirect(url: string, status: number = 302): VattsResponse {
        return new VattsResponse().redirect(url, status);
    }

    /**
     * Cria uma resposta 404
     */
    static notFound(message: string = 'Not Found'): VattsResponse {
        return VattsResponse.text(message, { status: 404 });
    }

    /**
     * Cria uma resposta 500
     */
    static error(message: string = 'Internal Server Error'): VattsResponse {
        return VattsResponse.text(message, { status: 500 });
    }

    /**
     * Cria uma resposta 400
     */
    static badRequest(message: string = 'Bad Request'): VattsResponse {
        return VattsResponse.text(message, { status: 400 });
    }

    /**
     * Cria uma resposta 401
     */
    static unauthorized(message: string = 'Unauthorized'): VattsResponse {
        return VattsResponse.text(message, { status: 401 });
    }

    /**
     * Cria uma resposta 403
     */
    static forbidden(message: string = 'Forbidden'): VattsResponse {
